...
def test_freesound_manager_load_token(mock_freesound_client, temp_auth_file):
    ...
    manager = FreesoundManager(client_id="id", client_secret="secret", auth_file=str(temp_auth_file))
...
@pytest.mark.asyncio
async def test_freesound_manager_search_bgm(mock_freesound_client):
    ...
    tags = ["loop", "music"]
    results = await manager.search_bgm(tags)
...
@pytest.mark.asyncio
async def test_freesound_manager_download_and_convert(mock_freesound_client, tmp_path):
    manager = FreesoundManager(client_id="id", client_secret="secret")
    ...
    with patch("pydub.AudioSegment.from_file") as mock_from_file, patch("tempfile.TemporaryDirectory") as mock_temp_dir:
        ...
        # Simulate sound.retrieve(temp_dir)
        ...
        final_path = await manager.download_and_convert(123, str(output_file))
        ...
        mock_audio.export.assert_called_once_with(str(output_file), format="wav", parameters=["-ar", "44100"])
