...
from vvr_scraper.job_models import JobManifest, RenderJob, ScrapeJob
...
def test_manifest_validation():
    ...
    # Valid Scrape Job
    valid_scrape = {"task": "crawl", "payload": {"slug": "test-story", "formats": ["epub"]}}
    ...
    valid_render = {
        "task": "render",
        "payload": {"manifest_path": "path/to/manifest.json", "output_path": "output.mp4"},
    }
    ...
    invalid_manifest = {"payload": {"slug": "test"}}
    with pytest.raises(ValidationError):
        JobManifest.model_validate(invalid_manifest)

    # Invalid Task Type
    invalid_task = {"task": "unknown", "payload": {}}
    with pytest.raises(ValidationError):
...
async def test_job_dispatching(job_worker, db_manager):
    ...
    with (
        ...
        # 1. Test Crawl Job
        scrape_data = {"task": "crawl", "payload": {"slug": "test-story"}}
        ...
        # 2. Test Render Job
        render_data = {"task": "render", "payload": {"manifest_path": "m.json", "output_path": "o.mp4"}}
...
async def test_job_error_logging(job_worker, db_manager):
    ...
    with patch("vvr_scraper.job_runner.execute_crawl_job", side_effect=Exception("Simulated failure")):
        scrape_data = {"task": "crawl", "payload": {"slug": "fail-story"}}
        scrape_job = JobManifest.model_validate(scrape_data)
        job_id = await db_manager.create_job("crawl", json.dumps(scrape_data))
