...
def mock_chapter_data():
    return [
        {"volume": "Vol 1", "chapters": [{"title": "Chapter 1", "url": "/c1"}, {"title": "Chapter 2", "url": "/c2"}]}
    ]


@pytest.mark.asyncio
async def test_run_scrape_task_success(mock_story_info, mock_chapter_data):
    req = DownloadRequest(slug="test-novel", formats=["EPUB", "PDF", "AD-MP3"], output_folder="/tmp/test_download")
    task_id = "task123"

    mock_db = AsyncMock()

    with (
        patch("vvr_scraper.web.routes.download.manager.broadcast", new_callable=AsyncMock) as mock_broadcast,
        ...
        patch(
            "vvr_scraper.web.routes.download.scrape_chapters",
            return_value={"https://valvrareteam.net/c1": [], "https://valvrareteam.net/c2": []},
        ),
            ...
            mock_broadcast.assert_any_call(
                {
                    "type": "error",
                    "task_id": "task123",
                    "error": "Quá nhiều chương tải thất bại: 2/2 (100%). Hủy xuất file.",
                }
            )
